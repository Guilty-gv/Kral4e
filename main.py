...
- KuCoin public API (async via SDK)
- Indicators: TA-Lib fallback to 'ta'
...
def combine_votes(votes_dict):
    votes_flat=[]
    [votes_flat.extend(v) for v in votes_dict.values()]
...
# ================ ANALYZE TASK ================
async def analyze_symbol(symbol: str, tf: str, session: None):
    key = (symbol, tf)
    now = datetime.utcnow()
    ...
    logger.info(f"Sending Telegram for {symbol} {tf} at price {last_price}")
    send_telegram(msg)
