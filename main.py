...
def suggested_prices(df, vote):
    ...
    if levels:
        if vote=="BUY":
            ...
            buy_price=max(below) if below else last*0.995
        elif vote=="SELL":
            above=[l for l in levels if l>last]
            sell_price=min(above) if above else last*1.005
...
async def main():
    ...
    for sym in all_symbols:
        try:
            info = market_client.get_symbol(sym)
            if info['enableTrading'] and info['quoteCurrency']=='USDT':
                supported_symbols.append(sym.replace("-", ""))
        except Exception as e:
            logger.warning("Failed to get info for symbol %s: %s", sym, e)
